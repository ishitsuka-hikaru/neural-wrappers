...
class Transformer:
	...
	#  must be applied for values in [0 : 240, 0 : 320] on the (480x640) label, not for [0 : 50, 0 :70]. The reshape is
	#  done at end. There may also be cases where the crop must be done on [0 : 50, 0 : 70], so leave it as an option.
	# TODO: rewrite transformer, for now no transforms :)
	...
	# 	print("applyTransform", npGetInfo(data))
	# 	newData, newLabels = self.transforms[transformName](data, labels)
	# 	print("applyTransform", npGetInfo(newData))
	...
	# 	print("applyTransform after resize", npGetInfo(newData))
